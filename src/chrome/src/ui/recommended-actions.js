...
const WP_TEMPLATE_SIGNAL_RE = /\b(template|templates|template part|site editor|theme|themes|customize|appearance|editor|patterns)\b/i;
...
function communicationSignal(pageInfo = {}) {
  return [
    pageInfo.url,
    pageInfo.title,
    pageInfo.description,
...
function isCommunicationThread(pageInfo = {}, host = '', path = '/') {
  ...
  if (EMAIL_HOST_RE.test(host)) {
    return /mail|inbox|message|thread|conversation|subject|reply|from|to/i.test(signal);
  }
...
function isWordPressTemplatePage(pageInfo = {}, path = '/') {
  ...
  const signal = `${pageInfo.title || ''} ${String(pageInfo.text || '').slice(0, 1200)}`;
  return WP_TEMPLATE_SIGNAL_RE.test(signal);
}
