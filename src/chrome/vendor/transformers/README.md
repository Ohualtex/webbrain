...
| `std::bad_alloc` from `OrtRun` even though `chrome://gpu` shows WebGPU is hardware accelerated | The asyncify wasm has no JSEP exports, so the WebGPU EP silently no-ops and inference runs on the WASM CPU backend, blowing the 2GB heap on the first ~500MB model. The `?.jsepOnCreateSession?.()` optional chaining in `ort.webgpu.mjs` hides the failure. | `offscreen.js` overrides `env.backends.onnx.wasm.wasmPaths` to `{mjs, wasm}` pointing at the `.jsep` files — that's the build with WebGPU EP bindings. If you re-vendor and skip that step, you'll re-hit this. |
