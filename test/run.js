...
test('chrome /record-full-screen is slash-only, Chrome-only, and hidden from the recording banner', () => {
  ...
  assert.match(offscreen, /addEventListener\('ended', \(\) => \{[\s\S]*?s\.captureEndedCleanupStarted = true;[\s\S]*?s\.recorder\.stop\(\);[\s\S]*?notifyCaptureEnded\(s\);[\s\S]*?releaseSession\(s\)\.catch/, 'chrome: capture-ended cleanup should notify background finalize and release mic/context immediately after stopping the recorder');
